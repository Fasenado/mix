...
const AnimalImg = styled.img`
  border-radius: 30px;
  width: 100%;
  min-width: ${props => (props.isInChat ? '280px' : 'auto')};
  min-height: ${props => (props.isInChat ? '280px' : 'auto')};
  object-fit: contain;
`;
...
class Animal extends React.Component {
  constructor(props) {
    ...
    this.isInChat = props.isInChat === true;
  ...
  updateAnimalMessage(animalData) {
    ...
    const animalName =
      animalData.prettyName || utils.capitalizeFirstLetter(animalData.name || '');
    const twitterMessage = [
      animalName,
      utils.CONTRACT_ADDRESS,
      '@' + utils.TWITTER_HANDLE
    ].join('\n');
  ...
  getTweetImage(animalData) {
    ...
    if ((animal1, animal2, animal3)) {
      ...
      return Promise.all([animalPromise])
        ...
        .catch(() => {
          // mixipedia API may be unavailable (e.g. local dev); use card data only
        });
    }
  }
        ...
        >
          <ChatDiv className="row">
            <AnimalContainer className="col s12 m10 offset-m1 image-div">
              <div
                className={this.isInChat ? '' : 'col s12 m8 offset-m2'}
                style={{
                  marginBottom: '10px',
                  maxWidth: this.isInChat ? '400px' : undefined,
                  margin: this.isInChat ? '0 auto' : undefined
                }}
              >
                <AnimalImg
            ...
            <div style={{ width: 'fit-content', margin: 'auto' }}>
              <AudioButton
                ...
                onClick={
                  this.isInChat
                    ? this.goAnimalUrl.bind(this)
                    : this.playAnimalSound.bind(this)
                }
              >
                <AudioButtonText>
                  {this.isInChat ? 'view' : 'deploy'}
                </AudioButtonText>
              </AudioButton>
            </div>
          </ChatDiv>
